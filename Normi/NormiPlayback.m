...
{
    buses[index].sound = &sounds[index];
    buses[index].sampleNumber = 0;
}
...
- (void) loadHits: (NSArray *)urls
{
    ...
    CFURLRef sourceURLArray[numSounds];
    
    for (int i = 0; i < urls.count; i++) {
        sourceURLArray[i] = (__bridge CFURLRef) urls[i];
    }
    
    // resize sounds to have all of the hits.
    sounds = (SoundData *) realloc (sounds, numSounds * sizeof(SoundData));
    if (sounds == NULL) { [self printErrorMessage: @"Error (re)allocating memory for sounds" withStatus: 1]; return;}

    
    for (int audioFile = 0; audioFile < numSounds; ++audioFile)  {
        
        SoundData sound;
        
        NSLog (@"readAudioFilesIntoMemory - file %@", sourceURLArray[audioFile]);
